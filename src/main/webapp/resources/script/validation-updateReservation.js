$(document).ready(() => {
	...
	$('#reservation-confirmed').on('click', e => { 
		...
		const sessionRenDate = $('#session-renDate').val();
		const sessionRenTime = $('#session-renTime').val();
		const sessionReturnDate = $('#session-returnDate').val();
		const sessionReturnTime = $('#session-returnTime').val();
		...
		if(location === "" || renDate === "" || returnDate === "" || renTime === null || returnTime === null) {
			...
			isValid = false;
			isConfirm = false;
		} else if(renDate === sessionRenDate && renTime === sessionRenTime && returnDate === sessionReturnDate && returnTime === sessionReturnTime) {
			isChange = false;
		} else {
			isValid = true;
			isConfirm = true;
